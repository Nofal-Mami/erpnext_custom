...
def execute(filters=None):
    ...
    period_list = get_period_list(int(filters.to_fiscal_year) - 1, filters.to_fiscal_year, filters.periodicity,
    ...
    for (index, config) in enumerate(configuration.get("financial_report_configuration_item")):
        ...
        if config.get('accounts') is None:
            ...
            prev, current, key1, key2 = compute_from_grouper(grouper,
